import { createReducerSlice, RootState } from './createReducerSlice';
...
export const createReduxStorage = (namespace: string) => {
  ...
  const useStorage = <T, R extends RootState>(key: string, initialValue: T): UseStorageApi<T> => {
    const dispatch = useDispatch();
    const action = useMemo(() => createAction<T>(key), [key]);
    const selector = useMemo(() => createSelector<T, R>(key, initialValue), [initialValue, key]);
    const value = useSelector<R, T>(selector);
